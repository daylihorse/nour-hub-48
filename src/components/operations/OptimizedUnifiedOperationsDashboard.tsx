...
const OptimizedUnifiedOperationsDashboard = memo(() => {
  const { alerts, stats, isLoading, error, criticalAlerts, totalInventoryValue, pendingOrdersCount } = useOptimizedOperationsData();
  const { getAverageRenderTime } = usePerformanceMonitor("UnifiedOperationsDashboard", process.env.NODE_ENV === 'development');
...
OptimizedUnifiedOperationsDashboard.displayName = "OptimizedUnifiedOperationsDashboard";

export default OptimizedUnifiedOperationsDashboard;
