...
const HorseModuleAccessCenter: React.FC = () => {
  const { moduleConfigs, toggleModule, toggleSubmodule, updateModuleConfig } = useModuleAccess();
  ...
  const moduleConfig = moduleConfigs["horses"];
  ...
  const confirmChange = () => {
    ...
    if (!pendingChange.submoduleId) {
      toggleModule("horses", pendingChange.newState);
    } else {
      toggleSubmodule("horses", pendingChange.submoduleId, pendingChange.newState);
    }
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingChange?.submoduleId ? 
                `Are you sure you want to ${pendingChange.newState ? 'activate' : 'deactivate'} this submodule? This will affect its visibility in the horse management dashboard.` :
