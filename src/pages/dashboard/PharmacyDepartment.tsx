...
import { BarChart3, Package, FileText, CreditCard, Users, Store, Home, Network, Hospital, Zap, FlaskRound, Shield, AlertTriangle, Bell, Calculator, Stethoscope } from "lucide-react";
const PharmacyDepartment = () => {
  ...
  return <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Pharmacy Department</h1>
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        <TabsList className="grid w-full grid-cols-8 lg:grid-cols-16 h-auto p-1 bg-muted">
          {tabConfigs.map(tab => <TabsTrigger key={tab.value} value={tab.value} className="flex flex-col items-center gap-1 p-2 h-auto text-xs data-[state=active]:bg-background data-[state=active]:text-foreground transition-all duration-200 data-[state=active]:scale-110 data-[state=active]:*:first-child:h-5 data-[state=active]:*:first-child:w-5 data-[state=active]:text-sm">
